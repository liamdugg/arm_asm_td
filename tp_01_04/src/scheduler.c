...
__attribute__((section(".kernel"))) uint32_t task_create(uint32_t rom_addr,uint32_t task_size ,task_priority_t priority, uint32_t ticks){
    ...
    while(i<MAX_TASKS){
        ...
        if(scheduler[i].state == DEAD){ // busco un hueco libre en el scheduler
            ...
            scheduler[i].sp_fiq = TASK_STACK + ((uint32_t)&_STACK_SIZE)*1;          // 0x80001000 + 0x0400      
            scheduler[i].sp_irq = TASK_STACK + ((uint32_t)&_STACK_SIZE)*2 -15*4;    // 0x80001000 + 0x0800 
            scheduler[i].sp_svc = TASK_STACK + ((uint32_t)&_STACK_SIZE)*3;          // 0x80001000 + 0x0C00 
            scheduler[i].sp_abt = TASK_STACK + ((uint32_t)&_STACK_SIZE)*4;          // 0x80001000 + 0x1000 
            scheduler[i].sp_und = TASK_STACK + ((uint32_t)&_STACK_SIZE)*5;          // 0x80001000 + 0x1400 
            scheduler[i].sp_usr = TASK_STACK + ((uint32_t)&_STACK_SIZE)*6;          // 0x80001000 + 0x1800 
            ...
            // seteo ttbr0 y pagino la tarea
            // la tarea 0 usa las tablas del kernel
            if(scheduler[i].tsk_id == 0)
...
__attribute__((section(".kernel"))) void task_page(task_t task){
    ...
    for(i=(uint32_t)&_VTBL_VMA_START; i<_VTBL_END; i+=(uint32_t)&_PAGE_SIZE)
        mmu_new_page(i,i,XN_AE, PL1_RW, task.ttbr0);
    ...
    for(i=task_base; i<(task_base+(uint32_t)&_TASK_SIZE); i+=(uint32_t)&_PAGE_SIZE){
        ...
        mmu_new_page((TASK_BASE+offset), i, XN_AE,PL1_RW, task.ttbr0);
    }
    ...
    mmu_new_page(TIMER0_BASE, TIMER0_BASE, XN_AE, PL1_RW, task.ttbr0);
    mmu_new_page(GICC0_BASE , GICC0_BASE , XN_AE, PL1_RW, task.ttbr0);
    mmu_new_page(GICC0_BASE , GICC0_BASE , XN_AE, PL1_RW, task.ttbr0);
...
__attribute__((section(".kernel"))) void task_preload_stack(uint32_t id){
    ...
    sp_irq--;
    *sp_irq = 0x80000000; // Cargo en la posicion donde voy a sacar el LR la direccion donde corren las tareas
...
__attribute__((section(".kernel"))) void sch_init(){
    
    // Arranco el scheduler corriendo la tarea idle
    // running_task = 0;
    // previous_task = 0;   
