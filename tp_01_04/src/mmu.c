...
__attribute__((section(".text"))) void mmu_sys_paging(){

}

/* --------------- GETTERS / SETTERS ---------------*/

__attribute__((section(".text"))) uint32_t mmu_get_ttl1_addr(void){
    ...
    if( ttbcr.T0SZ == 0 )
        ttl1_addr = ttbr0.ttbr0 & 0xFFFFC000;
...
__attribute__((section(".text"))) TTBR0 mmu_get_ttbr0(void){
    TTBR0 ttbr0;
...
__attribute__((section(".text"))) void mmu_set_ttbr0(uint32_t table_base){
    
    table_base &= (0xFFFFC000); 
    
    asm("ldr r0,%0" : "=m"(table_base));
    asm("ISB");
    asm("DSB");
...
__attribute__((section(".text"))) void mmu_ttl1_clear(uint32_t* ttl1_addr){
    for(int i=0; i < TTL1_ENTRIES; i++)
        ttl1_addr[i] = INVALID_DESCRIPTOR;
}
